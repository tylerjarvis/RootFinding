...
def quadratic_check_2D(test_coeff, intervals, change_sign, tol):
    ...
    # Note: Overhead for instantiating a NumPy array is too costly for small arrays, so the second sum here is faster than using numpy
    other_sum = np.sum(np.abs(test_coeff)) - sum([fabs(coeff) for coeff in c])
    ...
    mask = change_sign.copy() ## TODO check if we can skip copying and just use change_sign

    for i, interval in enumerate(intervals):
        
        if mask[i]:   # If change_sign true for this interval,
            continue  # then move to the next interval.
        ...
        if extreme_points[0]*extreme_points[1]<0:  # check for sign change
            mask[i] = True
            continue                               # Done with this subinterva
        extreme_points.append(eval_func(interval[0][0], interval[1][1]))
        if extreme_points[-2]*extreme_points[-1]<0:  #sign change
            mask[i] = True
            continue
        ...
        if c[5] != 0:
            ...
            if interval[0][1] < y < interval[1][1]:
                ...
                if extreme_points[-1]*extreme_points[-2] < 0:  # check for sign change
                    mask[i] = True
                    continue
            x = interval[1][0]
            y = -(c[2] + c[4]*x)/cc5
        ...
        target = other_sum + tol
        mask[i] = (min(extreme_points) < target
                and max(extreme_points) > -target)
