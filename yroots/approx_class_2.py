...
class M_maker:
    def __init__(self,f,a,b,deg,return_inf_norm=False,rel_approx_tol=1.e-15, abs_approx_tol=1.e-12):
        ...
        if self.return_inf_norm == True:
            ...
            self.err = np.sum(np.abs(self.M2[slice_top(self.M.shape)] - self.M))
        else:
    ...
    def error_test(self,error,rel_approx_tol,abs_approx_tol,inf_norm): 
        """
    ...
    def find_good_deg(self,f,deg,dim,a,b):
        """
        Find's the right degree with which to approximate on the interval.
...
def test_roots_1_1():
    ...
    print(f_approx.deg,g_approx.deg)
