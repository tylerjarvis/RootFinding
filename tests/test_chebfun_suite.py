...
def norm_pass_or_fail(yroots, roots, tol=2.220446e-13):
...
def residuals_pass_or_fail(funcs, roots, tol=1.e-13):
    """ Determines whether the roots given pass or fail the test according
...
def pass_or_fail(funcs, yroots, roots, test_num, test_type="norm"):
    ...
    ----------
        ...
        test_num : float or string
            ...
            "9.2" or 9.2.
        test_type : string
            What type of test to use to determine pass or fail.
             - "norm" -- norm_pass_or_fail
             - "residual" -- residual_pass_or_fail

    Raises
    ------
        AssertionError if len(yroots) != len(roots)
        ValueError if test_type is not "norm" or "residual"
    """
    ...
    if test_type == 'norm':
        ...
        assert residuals_pass_or_fail(funcs, yroots), "Test " + str(test_num) + " failed."
...
def test_roots_1():
    ...
    # Test 1.2
    # TODO No marching squares output.
...
def test_roots_2():
    ...
    # assert pass_or_fail(yroots, m_sq_roots), "Test 2.5 fails."


#def test_roots_3():
    
    # Test 3.1
    # TODO No MS roots to test against.
    # f = lambda x,y: ((x-.3)**2+2*(y+0.3)**2-1)
    # g = lambda x,y: ((x-.49)**2+(y+.5)**2-1)*((x+0.5)**2+(y+0.5)**2-1)*((x-1)**2+(y-0.5)**2-1)
    # yroots = solve([f,g],[-1,-1],[1,1])
    # m_sq_roots = np.array()

    # pass_or_fail([f,g], yroots, m_sq_roots, 3.1)
    ...
    # pass_or_fail([f,g], yroots, m_sq_roots, 3.2)
...
def test_roots_6():
    # # Test 6.1
    # # TODO No MS roots to compare to.
    # f = lambda x,y: (y - 2*x)*(y+0.5*x)
    # g = lambda x,y: x*(x**2+y**2-1)
    # yroots = solve([f,g],[-1,-1],[1,1])
    # m_sq_roots = np.array()

    # pass_or_fail([f,g], yroots, m_sq_roots, 6.1)
    ...
    # pass_or_fail([f,g], yroots, m_sq_roots, 6.2)
...
def test_roots_7():
    ...
    # yroots = solve([f,g],[-1,-1],[1,1])
    # m_sq_roots = np.loadtxt('tests/chebfun_test_output/cftest7_2ms.csv',delimiter=',')

    # pass_or_fail([f,g], yroots, m_sq_roots, 7.2)
    ...
    # pass_or_fail([f,g], yroots, m_sq_roots, 7.3)
...
#     pass_or_fail([f,g], yroots, m_sq_roots, 10.1
