    ...
    while np.any(poly.coeff):
        ...
        for divisor in divisors:
            ...
            if divides(divisor.lead_term, poly.lead_term):
                ...
                poly_coeff, poly_to_subtract_coeff = poly.match_size(poly.coeff, poly_to_subtract.coeff)
                new_coeff = poly_coeff - \
                    (poly.lead_coeff/poly_to_subtract.lead_coeff)*poly_to_subtract_coeff
        ...
        if not divisible:
            remainder_coeff, poly_coeff = remainder.match_size(remainder_coeff, poly.coeff)

            polyLT = tuple(poly.lead_term)

            # Add lead term to remainder
            #remainder_coeff = remainder.coeff
            remainder_coeff[polyLT] = poly_coeff[polyLT]
            #remainder.__init__(remainder_coeff, lead_term = remainder.lead_term, clean_zeros = False)
